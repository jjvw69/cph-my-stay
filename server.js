...
const OWNER_HTML = fs.readFileSync(path.join(__dirname, 'owner.html'));
const PROPERTY_CONSOLE_HTML = fs.readFileSync(path.join(__dirname, 'property-console.html'));
...
async function route(req,res){
  ...
  if(url.startsWith('/api/staff/')){
    ...
    if(m==='GET' &&url==='/api/staff/bootstrap') return sendJSON(res,200,{ok:true,villas:store.listVillas(),addons:store.listServicesForStaff(),explore:store.EXPLORE_SCENES,exploreOptions:store.EXPLORE_BOOK,concierges:store.CONCIERGES,yachtCatalog:store.YACHT_CATALOG,serviceOptions:store.SERVICE_OPTIONS,provisioningOptions:store.PROVISIONING_OPTIONS,dietTags:store.DIET_TAGS,bookingSources:store.BOOKING_SOURCES,bookingSourcePartners:store.BOOKING_SOURCE_PARTNERS,serviceBookedVia:store.SERVICE_BOOKED_VIA,calendarFeeds:Object.keys(CAL_FEEDS).map(k=>({id:k,label:CAL_FEEDS[k],url:APP_URL+'/calendar/'+CALENDAR_TOKEN+'/'+k+'.ics'}))});
    ...
    if(ivS&&m==='POST'){ const action=ivS[3]; const status=action==='send'?'sent':action; const _prevSt=store.getStay(ivS[1]); const _prevInv=_prevSt&&(_prevSt.invoices||[]).find(x=>x.id===ivS[2]); const _prevStatus=_prevInv?_prevInv.status:''; const it=store.setInvoiceStatus(ivS[1],ivS[2],status); if(it){ /* Only a FIRST send (draft→sent) notifies the guest. Re-sending an already-sent invoice, or reverting a paid invoice back to unpaid, must never re-notify. */ if(action==='send' && _prevStatus==='draft'){ const st=store.getStay(ivS[1]); if(st&&st.status==='published'){ const tot=store.invoiceTotal(it); notifyGuest(st,'Your invoice is ready',['Your concierge has sent you an invoice — open My Stay to review and settle it.','',`${it.title}: $${tot.toLocaleString('en-US')}${it.dueBy?' · due '+it.dueBy:''}`].join('\n')); } } broadcastStaff({type:'update'}); } return it?sendJSON(res,200,{ok:true,invoice:it}):sendJSON(res,404,{ok:false,error:'Not found'}); }
  ...
  if (url === '/owner') return sendHTML(res, OWNER_HTML);
  if (url === '/property-console') return sendHTML(res, PROPERTY_CONSOLE_HTML);
