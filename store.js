...
const EXPLORE_BOOK = {
  ...
  tennis:         { label:'What would you like?', choices:[{label:'Tennis — court rental',rate:'US$ 39–49'},{label:'Tennis — private lesson',rate:'US$ 75–159 / h'},{label:'Tennis — group clinic',rate:'US$ 35–45'},{label:'Padel — court rental',rate:'US$ 45–49'},{label:'Padel — lesson',rate:'from US$ 60'},{label:'Racquet / paddle rental',rate:'US$ 10–19'},{label:'Other',rate:''}] },
  equestrian:     { label:'What would you like?', choices:[{label:'Horseback ride',rate:'US$ 40–65 p/p'},{label:'Pony ride (kids 4–6)',rate:'US$ 50'},{label:'Riding / jumping lesson',rate:'US$ 100–150 p/p'},{label:'Polo lesson',rate:'US$ 120–200 p/p'},{label:'Donkey polo',rate:'US$ 65 p/p'},{label:'Ranch horseback tour',rate:'US$ 170–255 p/p'},{label:'Other',rate:''}] },
  shooting:       { label:'What would you like?', choices:[{label:'Skeet / Trap round',rate:'US$ 55'},{label:'Five-stand round',rate:'US$ 50'},{label:'Sporting clays round',rate:'US$ 115–180'},{label:'Flush (group)',rate:'US$ 125–250'},{label:'Lesson',rate:'US$ 25–100'},{label:'Bird shooting',rate:'from US$ 45 / bird'},{label:'Other',rate:''}] },
...
function saveStay(id, patch) {
  ...
  if ('rooms' in patch) s.rooms = sanitizeRooms(patch.rooms);
  if ('carts' in patch) s.carts = sanitizeCarts(patch.carts);
  if ('staffLog' in patch) s.staffLog = sanitizeStaffLog(patch.staffLog);
...
/** Shape guard for dining reservations (staff-only). */
function sanitizeReservations(list) {
  ...
  return list.slice(0, 80).map(r => ({
    ...
    pax: Math.max(0, Math.min(300, parseInt(r && r.pax, 10) || 0)),
    menu: norm(r && r.menu).slice(0, 120),
    cost: norm(r && r.cost).slice(0, 40),
    deposit: norm(r && r.deposit).slice(0, 40),
...
/** Shape guard for the room & bed assignment board (staff-only). */
function sanitizeRooms(list) {
  if (!Array.isArray(list)) return [];
  return list.slice(0, 40).map(r => ({
    id: (norm(r && r.id).slice(0, 40)) || genId(),
    name: norm(r && r.name).slice(0, 60),
    beds: norm(r && r.beds).slice(0, 60),
    guests: norm(r && r.guests).slice(0, 200),
    dates: norm(r && r.dates).slice(0, 60),
    note: norm(r && r.note).slice(0, 200),
  })).filter(r => r.name || r.beds || r.guests);
}
/** Shape guard for the golf-cart assignment roster (staff-only). */
function sanitizeCarts(list) {
  if (!Array.isArray(list)) return [];
  return list.slice(0, 80).map(r => ({
    id: (norm(r && r.id).slice(0, 40)) || genId(),
    name: norm(r && r.name).slice(0, 80),
    seats: Math.max(0, Math.min(20, parseInt(r && r.seats, 10) || 0)),
    house: norm(r && r.house).slice(0, 60),
    note: norm(r && r.note).slice(0, 120),
  })).filter(r => r.name || r.seats);
}
/** Shape guard for the staff overtime log (staff-only). */
function sanitizeStaffLog(list) {
  if (!Array.isArray(list)) return [];
  return list.slice(0, 120).map(r => ({
    id: (norm(r && r.id).slice(0, 40)) || genId(),
    date: norm(r && r.date).slice(0, 20),
    role: norm(r && r.role).slice(0, 60),
    hours: Math.max(0, Math.min(1000, parseFloat(String((r && r.hours) == null ? '' : r.hours).replace(/[^0-9.]/g, '')) || 0)),
    rate: Math.max(0, Math.min(100000, parseFloat(String((r && r.rate) == null ? '' : r.rate).replace(/[^0-9.]/g, '')) || 0)),
    note: norm(r && r.note).slice(0, 160),
  })).filter(r => r.role || r.hours || r.rate);
}
